...
defineTypeNameAndDebug(cuFoamRichardson, 0);

// 对称的(symmetric)
lduMatrix::solver::addsymMatrixConstructorToTable<cuFoamRichardson>
        addRichardsonSolverSymMatrixConstructorToTable_;

} // ~namespace Foam

Foam::cuFoamRichardson::cuFoamRichardson(
...
Foam::solverPerformance Foam::cuFoamRichardson::solve(
        scalarField &psi,
    ...
    Richardson(&d_x, d_A, d_b, error * norm_factor, 0.0, 30, max_steps, &res);

    // 将结果传回OpenFOAM
    d_x.CopyToHost(x);
    std::copy(x.begin(), x.end(), psi.begin());

    solverPerformance solverPerf("cuFoamRichardson", fieldName());
