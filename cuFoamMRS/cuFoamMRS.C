...
defineTypeNameAndDebug(cuFoamMRS, 0);

// 对称的(symmetric)
lduMatrix::solver::addsymMatrixConstructorToTable<cuFoamMRS>
        addMRSSolverSymMatrixConstructorToTable_;

// 不对称的(asymmetric)
lduMatrix::solver::addasymMatrixConstructorToTable<cuFoamMRS>
        addMRSSolverAsymMatrixConstructorToTable_;

} // ~namespace Foam

Foam::cuFoamMRS::cuFoamMRS(
...
Foam::solverPerformance Foam::cuFoamMRS::solve(
        scalarField &psi,
    ...
    // 将预处理矩阵转换成CSR格式
    CPU::SparseCSR<ValueType, IndexType> invM = A.JacobiPrecondition();
    ...
    SingleGPU::SparseCSR<ValueType, IndexType> d_invM{invM};
    ...
    MRS(&d_x, d_A, d_b, d_invM, error * norm_factor, max_steps, &res);

    // 将结果传回OpenFOAM
    d_x.CopyToHost(x);
    std::copy(x.begin(), x.end(), psi.begin());

    solverPerformance solverPerf("cuFoamMRS", fieldName());
