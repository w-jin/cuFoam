...
defineTypeNameAndDebug(cuFoamMR, 0);

// 对称的(symmetric)
lduMatrix::solver::addsymMatrixConstructorToTable<cuFoamMR>
        addMRSolverSymMatrixConstructorToTable_;

// 不对称的(asymmetric)
lduMatrix::solver::addasymMatrixConstructorToTable<cuFoamMR>
        addMRSolverAsymMatrixConstructorToTable_;

} // ~namespace Foam

Foam::cuFoamMR::cuFoamMR(
...
Foam::solverPerformance Foam::cuFoamMR::solve(
        scalarField &psi,
    ...
    MR(&d_x, d_A, d_b, error * norm_factor, max_steps, &res);

    // 将结果传回OpenFOAM
    d_x.CopyToHost(x);
    std::copy(x.begin(), x.end(), psi.begin());

    solverPerformance solverPerf("cuFoamMR", fieldName());
